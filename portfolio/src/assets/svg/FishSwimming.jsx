...
const FishSwimming = ({ isClicked, errorLoad }) => {
    ...
    // useState per gestire il ritardo della bolla di dialogo
    const [delaySpeechBubble, setDelaySpeechBubble] = useState(false);

    // Imposta un timeout per mostrare la bolla di dialogo dopo 5 secondi
    setTimeout(() => {
  ...
  return (
    <div className="w-full h-[400px] absolute bottom-0">
        <div className='absolute left-[60%] top-[90%] md:left-[55%] lg:top-[88%] lg:left-[53%] xl:top-[80%]'>
            {delaySpeechBubble && !isClicked && (
                errorLoad ? (
                    <SpeechBubble speed={150}>I'm sorry, the projects are currently unavailable</SpeechBubble>
                ) : (
                    <SpeechBubble speed={150}>What a beautiful day! Click on the lanterns to see the projects</SpeechBubble>
            ))}
        </div>
        <svg viewBox="0 0 900 300" className="w-full h-full" preserveAspectRatio="xMidYMax slice" xmlns="http://www.w3.org/2000/svg">
            ...
            {isClicked ? (
                <motion.g
                    initial={{ x: -1000, y: 55 }}
                    animate={{ x: [-1000, 1000], y: [55, 25, -130, 35, 55], rotateZ: [0, -25, 35, 0] }}
                    transition={{ duration: 6, repeat: 4 }}
                >
                    ...
                    <motion.path
                        ...
                        animate={{ rotateY: [45, 35, 25, 15, 5, -15, -25, -35, -45], scale: [1, 1.025, 1.05, 1.075, 1.1, 1.075, 1.05, 1.025, 1] }}
                        transition={{ duration: 1.8, repeat: 6, ease: 'easeIn' }}
                    />
